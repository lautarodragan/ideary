 ...
 * @version     $Id: sh404sefsocial.php 2351 2012-06-12 09:29:35Z silianacom-svn $
 */
  ...
  private function _sh404sefGetSocialButtons( $sefConfig, $url = '', $context = '', $content = null, $imageSrc = '', $imageDesc = '') {
    ...
    if($this->_params->get('enablePinterestPinIt', 1) && in_array( 'pinterestpinit', $this->_enabledButtons)) {
      ...
      if(!empty( $imageSrc)) {
        ...
        $buttonsHtml .= '<a href="http://pinterest.com/pin/create/button/?url=' . urlencode( $url)
        . '&media=' . urlencode( $imageSrc)
        . (empty( $imageDesc) ? '' : '&description=' . urlencode( $imageDesc) ) . '" '
        . 'class="pin-it-button" count-layout="' . $this->_params->get( 'pinItCountLayout', 'none') . '">'
        . $this->_params->get( 'pinItButtonText', 'Pin it')
        . '</a>';
