...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$.get("<?=$this->baseurl?>/index.php?option=com_content&task=get_recommendations", {
			"article_id": <?=$this->item->id?>
				...
				$("#recommended-texts span.article a").click(function(){
					ga('send', 'event', 'system-recommendation', <?=$this->item->id?>, recommendation.recommendation.title);
				});
			});
    ...
    $(document).ready(function(){
		tinymce.init({
			...
			content_css: "templates/beez_20/css/tinymce_comments.css?v=2",
			setup : function(ed) {
		...
		function publishComment() {
            ...
            var commentL = $.trim(tinyMCE.activeEditor.getContent({format: "text"}));
			var commentLength = commentL.length;
					...
					$.post(
						...
						function(data) {
							...
							if(result){
								...
								$('#comments').prepend(
									'<div class="comment-applauses"><img src="${imgPath}" /><span>${username} aplaudió </span></div>'
									.replace('${imgPath}', comment_img2)
...
<div id="text-view">
    ...
    <div id="text-view-text-header" >
		<div id="text-view-category-color" style="background: <?= $this->item->first ? "#E02424" : "#616381" ?>;"></div>
   ...
   <div id="text-view-text-adjustments">
    ...
    <div id="social-icons-container">
        ...
        <div id="social-icons">
			...
			<a title="Compartir en Twitter" href="https://twitter.com/share" target="_blank" id="tw-icon" class="social-icon"  data-via="ideary" data-lang="es" data-count="none" data-dnt="true"></a>

			<a title="Compartir en Google+" href="https://plus.google.com/share?url=<?= urlencode("http://".$_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"]) ?>" target="_blank" id="gp-icon" class="social-icon"  ></a>
    ...
    <div id="comment-container">
        ...
        <div id="comments">
            <? 
            	...
            	for($i = 0; $i < count($this->applausesAndCommentsIndex); $i++) {
            		$applausesAndComment = $this->applausesAndCommentsIndex[$i];
            		if ($applausesAndComment[0] == 1) {
            			$comment = $this->comments[$applausesAndComment[1]];
						...
						// UGLY HACK: "Old" comments were plain text, new ones are WYSIWYG and include <p>'s
						if ($comment->id < 2419) { 
	            ...
	            <?  } else { 
	            		if ($i + 1 < count($this->applausesAndCommentsIndex) && $this->applausesAndCommentsIndex[$i + 1][0] == 0) {
	            			$applauseGroupper[] = $this->applauses[$applausesAndComment[1]];
	            			continue;
	            		} else if (count($applauseGroupper) > 0) {
	            			$applauseGroupper[] = $this->applauses[$applausesAndComment[1]];
	            			$names = "";
	            			$images = "";
	            			$groupperCounter = 0;
	            			foreach ($applauseGroupper as $groupedApplause) {
		            			$applauseAuthorLink = ($groupedApplause->userId != $user->id) ?
									JRoute::_('index.php?option=com_contact&view=public&id='.$groupedApplause->userId) :
									JRoute::_('index.php?option=com_users&view=profile&user_id='.$groupedApplause->userId);

	            				$names .= $groupedApplause->name . ", ";
	            				$images .= "<a href='$applauseAuthorLink'>" . ideary::getUserImage($groupedApplause->userId, "50", $groupedApplause->name) . '</a>';
	            				$groupperCounter++;
	            				if ($groupperCounter >= 5)
	            					break;
	            			}
	            			$names = preg_replace('~,(?!.*,)~', '', $names);
	            			if ($groupperCounter >= count($applauseGroupper))
	            				$names = preg_replace('~,(?!.*,)~', ' y ', $names);

	            			?>
	            			<div class="comment-applauses">
			            		<?= $images ?> <? if ($groupperCounter < count($applauseGroupper)) echo "<span class='and_more_applauders'>+" . (count($applauseGroupper) - $groupperCounter) . "</span>"; ?>
			            		<span><?= $names ?> <? if ($groupperCounter < count($applauseGroupper)) echo "y otros " . (count($applauseGroupper) - $groupperCounter); ?> aplaudieron </span>
			            	</div>
	            			<?
	            			...
	            			$applauseAuthorLink = ($applause->userId != $user->id) ?
								JRoute::_('index.php?option=com_contact&view=public&id='.$applause->userId) :
								JRoute::_('index.php?option=com_users&view=profile&user_id='.$applause->userId);
	            		
	  			?>
            	<div class="comment-applauses">
            		<a href="<?=$applauseAuthorLink?>"><?= ideary::getUserImage($applause->userId, "50", $applause->name) ?></a>
            		<span><?= $applause->name ?> aplaudió </span>
            	</div>
            <? 		
            			}
        			}
	...
	<div id="recommended-texts" style="visibility: hidden; " >
		...
		<div>¿Leíste <span class="article"><a href=""></a></span><span> de </span><span class="author"></span>?</div>
