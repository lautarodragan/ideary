...
class ContentModelArticle extends JModelItem
{
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				$query->select($this->getState(
					'item.select', 'a.id, a.allow_comments, a.asset_id, a.title, a.alias, a.title_alias, a.introtext, a.fulltext, a.editor_font_scale, a.editor_font_size, a.first, a.pinned, ' .
    ...
    public function is_clapped($text_id, $user_id){

        $db = $this->getDbo();

        $query = 'SELECT COUNT(*) FROM text_applauses WHERE text_id='.$text_id.' AND user_id='.$user_id;
	...
	public function getRandomArticleId(){
        ...
        $query = "select id from #__content where state = 1 ORDER BY RAND() limit 1";
