 ...
 * @since 1.5
 */
class ContentModelRanking extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $_context = 'com_content.ranking';
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				$query->select($this->getState(
					'item.select', 'a.id, a.allow_comments, a.asset_id, a.title, a.alias, a.title_alias, a.introtext, a.fulltext, ' .
    ...
    public function clap($text_id, $user_id){

        $db = $this->getDbo();

        $query = 'INSERT INTO #__applauses ( text_id, user_id, created_at )' .
            ' VALUES ( '.(int) $text_id.', '.(int) $user_id.', \''.date_create()->format('Y-m-d H:i:s').'\' )';
    ...
    public function getNotification($user_id, $notification_type){
        ...
        $query = "SELECT nt.id, ntu.send_email FROM #__notification_types_users AS ntu JOIN #__notification_types AS nt ON (nt.id = ntu.notification_type_id)".
            " WHERE ntu.user_id=".$user_id." AND nt.type='".$notification_type."' LIMIT 1";
    ...
    public function addToFavotites($text_id, $user_id){
        $db = $this->getDbo();

        $query = 'INSERT INTO #__favorites ( text_id, user_id, date, fav )' .
            ' VALUES ( '.(int) $text_id.', '.(int) $user_id.', \''.date_create()->format('Y-m-d H:i:s').'\', true )';
    ...
    public function undoAddToFavotites($text_id, $user_id){
        $db = $this->getDbo();

        $query = 'DELETE FROM #__favorites WHERE text_id='.(int) $text_id.' AND user_id='.(int) $user_id.' AND fav=true';
    ...
    public function addToSaved($text_id, $user_id){
        $db = $this->getDbo();

        $query = 'INSERT INTO #__favorites ( text_id, user_id, date, fav )' .
            ' VALUES ( '.(int) $text_id.', '.(int) $user_id.', \''.date_create()->format('Y-m-d H:i:s').'\', false )';
    ...
    public function unarchive($text_id, $user_id){
        $db = $this->getDbo();

        $query = 'DELETE FROM #__favorites WHERE text_id='.(int) $text_id.' AND user_id='.(int) $user_id.' AND fav=false';
    ...
    public function denounce($text_id, $user_id){
        $db = $this->getDbo();

        $query = 'INSERT INTO #__complaints ( text_id, user_id, created_at )' .
            ' VALUES ( '.(int) $text_id.', '.(int) $user_id.', \''.date_create()->format('Y-m-d H:i:s').'\' )';
    ...
    public function findById($id){
        ...
        $query = 'SELECT * FROM #__content WHERE id='.(int) $id;
    ...
    public function getText($text_id){
        ...
        $query = "SELECT * FROM #__content WHERE id=".(int)$text_id;
    ...
    public function searchTags($search){
        ...
        $query = "SELECT t.id, t.name FROM #__tags t ".
            "JOIN #__languages l ON (t.language_id = l.lang_id) ".
            "WHERE l.lang_code='".mysql_real_escape_string($lang_code)."' AND t.name LIKE '".mysql_real_escape_string($search)."%' ".
            "ORDER BY t.name";
    ...
    public function existTag($search){
    ...
    public function deleteTag($tag_id, $text_id){
        $db = $this->getDbo();

        $query = 'DELETE FROM #__tags_texts WHERE tag_id='.(int) $tag_id.' AND text_id='.(int) $text_id;
    ...
    public function deleteImage($text_id){
		$db = $this->getDbo();

        $query = 'DELETE FROM #__content_images WHERE text_id='.(int) $text_id;
