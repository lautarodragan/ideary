...
class UsersControllerReset extends UsersController
{
	...
	public function request()
	{
		...
		if ($return instanceof Exception) {
			...
			if (strstr($message,'No valid image')===false){
			...
			//	die("1");
				...
				$route	= 'index.php?option=com_content&view=featured';
				//die("2");
				$app->setUserState('users.login.form.resetpass', true);
			...
			$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
			//$route	= 'index.php?option=com_users&view=reset&layout=confirm'.$itemid;
			$route	= 'index.php?forgot=confirm';
			// Proceed to step two.
	...
	function confirm()
	{
		...
		//	$route	= 'index.php?option=com_users&view=reset&layout=confirm'.$itemid;
			...
			$route	= 'index.php?forgot=complete';
	...
	public function complete()
	{
		...
		if ($return instanceof Exception) {
			...
			$itemid = UsersHelperRoute::getResetRoute();
			$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
			$route	= 'index.php?option=com_users&view=reset&mal=1&layout=complete'.$itemid;

			//viene aca y esta bien...
			...
			$route	= 'index.php?option=com_content&view=featured&forgot=success'; // van al home
