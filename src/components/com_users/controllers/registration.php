...
class UsersControllerRegistration extends UsersController
{
	...
	public function activate()
	{
		...
		//esto es para loguearlo solo
		$db = JFactory::getDbo();
		$query = 'SELECT username from text_users where activation = "'.$token.'"';
		$db->setQuery($query);
		//echo $query;
		$result = $db->loadObject();
		
		$token = JRequest::getVar('token', null, 'request', 'alnum');
		$pass = JRequest::getVar('pwwr');
		...
		$exito = $app->login($credentials, $options);
		...
		*/
		...
		if ($return === false) {
			...
			if (strstr($model->getError(),'No valid image')===false){
				...
				//$this->setRedirect('index.php?errorcode=111');
				return false;				 
			}else{
				$this->setRedirect('index.php?codesuccess=111');
			}
		}
...
//			$this->setRedirect(JRoute::_('index.php?codesuccess=111'));
			$this->setRedirect('index.php?codesuccess=111');
		}
	...
	public function register()
	{
		...
		if ($return === false) {
			...
			$return = ideary::registerDoppler($data); 	//register doppler
